...
// H1: centered with accent underline
function h1(text) {
  return new docx.Paragraph({
    heading: docx.HeadingLevel.HEADING_1,
    alignment: docx.AlignmentType.CENTER,
    spacing: { before: 400, after: 200, line: LINE_SPACING },
    border: { bottom: { style: docx.BorderStyle.SINGLE, size: 4, color: P.accent, space: 6 } },
    children: [new docx.TextRun({ text, font: FONT, size: 32, bold: true, color: P.primary })],
  });
}

// H2: left-aligned with left accent bar
function h2(text) {
  return new docx.Paragraph({
    heading: docx.HeadingLevel.HEADING_2,
    alignment: docx.AlignmentType.LEFT,
    spacing: { before: 300, after: 140, line: LINE_SPACING },
    indent: { left: 200 },
    border: { left: { style: docx.BorderStyle.SINGLE, size: 12, color: P.primary, space: 8 } },
...
// BoldLead bullet: justified alignment
...
// Decorative section separator between major H1 sections
function sectionDivider() {
  return new docx.Paragraph({
    alignment: docx.AlignmentType.CENTER,
    spacing: { before: 200, after: 200 },
    indent: { left: 3000, right: 3000 },
    border: { top: { style: docx.BorderStyle.SINGLE, size: 4, color: P.accent, space: 4 } },
    children: [],
  });
}

// Card wrapper for tables — framed box with padding and background
function cardWrap(innerTable) {
  const cardBorders = {
    top: { style: docx.BorderStyle.SINGLE, size: 6, color: P.primary },
    bottom: { style: docx.BorderStyle.SINGLE, size: 6, color: P.primary },
    left: { style: docx.BorderStyle.SINGLE, size: 6, color: P.primary },
    right: { style: docx.BorderStyle.SINGLE, size: 6, color: P.primary },
    insideHorizontal: { style: docx.BorderStyle.NONE, size: 0, color: "auto" },
    insideVertical: { style: docx.BorderStyle.NONE, size: 0, color: "auto" },
  };
  return new docx.Table({
    width: { size: 96, type: docx.WidthType.PERCENTAGE },
    alignment: docx.AlignmentType.CENTER,
    rows: [
      new docx.TableRow({
        cantSplit: true,
...
// Inner table creator with enhanced alternating row shading
    ...
    (row, ri) =>
      new docx.TableRow({
        ...
        children: row.map((cell, i) =>
          new docx.TableCell({
            width: { size: pcts[i], type: docx.WidthType.PERCENTAGE },
            shading: shading(ri % 2 === 0 ? "FFFFFF" : "F8F6F1"),
            borders: NO_BORDERS,
            margins: CM,
            children: [
              new docx.Paragraph({
    ...
    ...lines.map(
      (line) =>
        new docx.Paragraph({
          ...
          children: [new docx.TextRun({ text: line, font: FONT, size: 76, bold: true, color: P.cover.titleColor })],
        })
    ),
    new docx.Paragraph({
      spacing: { before: 200, after: 200 },
          ...
          new docx.TableRow({
            ...
            children: [
              new docx.TableCell({
                ...
                borders: NO_BORDERS,
                margins: { top: mm2twip(20), bottom: mm2twip(20), left: mm2twip(30), right: mm2twip(30) },
                children,
              }),
            ],
          }),
...
function buildTocSection() {
  return {
    properties: {
      page: { margin: { top: 1440, bottom: 1440, left: 1701, right: 1701 }, size: { width: 11906, height: 16838 } },
    },
    ...
    styles: {
      default: {
        ...
        heading2: {
          ...
          paragraph: { spacing: { before: 300, after: 140, line: LINE_SPACING } },
        },
        heading3: {
...
async function main() {
  ...
  console.log("Building enhanced document...");
